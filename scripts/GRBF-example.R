...
predictions <- rbind(
  predictions,
  data.frame(time = x, pred = grbf.mod$coefficients[3] * grbfs$grbf2, grbf = 2)
)
predictions <- rbind(
  predictions,
  data.frame(time = x, pred = grbf.mod$coefficients[4] * grbfs$grbf3, grbf = 3)
)
predictions <- rbind(
  predictions,
  data.frame(time = x, pred = grbf.mod$coefficients[5] * grbfs$grbf4, grbf = 4)
)
...
ggplot(data = df, aes(x = x, y = y)) +
  ...
  scale_color_manual(
    values = c("#17BEBB", "#E7BBE3", "#0C6291", "#880D1E"),
    name = "Scaled Gaussan\nradial basis\nfunctions",
    labels = c("", "", "", "")
  ) +
  geom_hline(
    yintercept = grbf.mod$coefficients[1],
    linetype = 3,
    color = "#F18701"
  ) +
  theme_minimal() +
  ...
  annotate("text", label = "Model intercept", x = 8, y = 2, color = "#F18701") +
  guides(color = guide_legend(nrow = 2, byrow = TRUE))


ggsave(
  filename = "paper/grbf-example.png",
  width = 8,
  height = 4.5,
  units = "in"
)
...
ggsave(
  filename = "paper/grbf-example.pdf",
  width = 8,
  height = 4.5,
  units = "in"
)
