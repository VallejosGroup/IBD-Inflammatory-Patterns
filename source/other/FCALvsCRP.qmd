...
for (G in 2:8) { # No G = 8 for FCAL
  ...
  column_ha <- HeatmapAnnotation(
    ...
    "FC cluster" = models.fcal[[G]]$pprob$class[models.fcal[[G]]$pprob$ids %in% ids.common],
    col = list("FC cluster" = col.vec, "CRP cluster" = col.vec)
  )
  ...
  heat <- Heatmap(comp.mat,
                  ...
                  heatmap_legend_param = list(
                    labels = c(
                      "Shares cluster for both FC and CRP",
                      "Shares cluster for only CRP",
                      "Shares cluster for only FC",
...
p1 <- ggplot(classes, aes(x = type,
  ...
  theme_minimal() +
  theme(axis.line = element_line(colour = "gray"))  +  theme(legend.position='bottom')
