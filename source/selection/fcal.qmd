...
This report describes how the latent class mixed models (LCMMs) were fitted.
Different models are explored to determine the most appropriate models for FCAL
and CRP.
...
as intercepts, and an autoregressive process is used to model correlation
between observations. 

Natural cubic splines have a few notable advantages [@Elhakeem2022]:
...
To determine the most appropiate number of knots for the natural cubic splines,
we consider 3, 4, and 5 knots. We only use six-cluster models for this
analysis as this reasonably captures heterogeneity without being too expensive
computationally.

Our previous work used 3 knots across a 5-year period. As we are now modelling
across a 7-year period, we may need to increase the number of knots to 4 to
ensure the model remains flexible. We also consider 2 knots in case a more
smoooth model is required.

We place the knots at their default location which is
at quantiles. We note that the number of
knots is reported by @harrell2015 to be much more important than knot location.
...
#| label: fig-grbf-k-2
#| fig-cap: "Cluster profiles for the six-cluster model assuming two knots. The vertical teal lines indicate knot location. Mean cluster profiles are denoted by the red curves"
knots.grbf <- readRDS(paste0(prefix, "cache/fcal/grbf/k=2/grbf-1.2.RDS"))
knots.grbf.list <- list()
knots.grbf.list[[6]] <- knots.grbf
png("paper/grbf-plots.png", width = 8, height = 7, units = "in", res = 300)
spaghettiPlot(fcal,
              knots.grbf.list,
              G = 6,
              log = TRUE,
...
#| fig-cap: "Cluster profiles for the six-cluster model assuming three knots. The vertical teal lines indicate knot location. Mean cluster profiles are denoted by the red curves"
knots.grbf <- readRDS(paste0(prefix, "/cache/fcal/grbf/k=3/grbf-1.2.RDS"))
knots.grbf.list <- list()
knots.grbf.list[[6]] <- knots.grbf
png("paper/grbf-plots.png", width = 8, height = 7, units = "in", res = 300)
spaghettiPlot(fcal,
...
knots.df <- tibble(Knots = c(2, 3, 4, "GRBF"),
...
rm(knots.3, knots.3.list, knots.4, knots.4.list, knots.grbf.list)
```

AIC and BIC both suggest a 4-knot specification to be optimal (@tbl-knot-stats). 
However, visual inspection of the mean cluster trajectories
([see above](#model-trajectories)) suggests the 4-knot approach results in
overfitting- especially around knot locations. This will also become more of
an issue as the number of clusters increase and additional sparsity is exhibited
for some clusters.

As such, we have elected to use natural cubic splines with 3 knots placed at 
quantiles. 
...
itted to generate initial starting values, (the "Initial LME" tab) and then a
grid search approach is used to converge the LCMMs towards a global maximum for
each assumed number of clusters based on maximum likelihood (the "LCMM" tab). 
...
found in our previous work [@constantine-cooke2023], and we expect to find more
clusters given our inclusion criteria is more relaxed.
...
From the alluvial plot comparing FCAL models (@fig-fcal-alluvial), we can see
...
another. From  @fig-fcal-alluvial, 6 clusters appear to be a reasonable choice. 
...
fisher.test(dict.fcal$diagnosis,
            ...
            B = 1e5) # replicates for Monte Carlo test 
```
...
```{R}
pred.df <- data.frame(calpro_time = seq(0, 7, by = 0.01),
                      diagnosis = "Crohn's Disease")
pred.df <- rbind(pred.df,
                 data.frame(calpro_time = seq(0, 7, by = 0.01),
...
for (g in 1:6) {
  ...
  p <- plot.df %>%
    ...
    ylim (3 , 8) + 
    theme_minimal() + scale_color_manual(values = c("#0E6BA8", "#EF6461"))
  p.list[[g]] <- p
} 
