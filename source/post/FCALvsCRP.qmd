...
fcal.pprob <- models.fcal[[10]]$pprob
crp.pprob <- models.crp[[10]]$pprob
...
preds.fcal <- predictY(models.fcal[[10]], newdata = new.data, draws = TRUE)
plot.fcal <- data.frame(time = calpro_time,
                        pred = preds.fcal$pred[, "Ypred_class1"],
                        class = "1")
for (class in as.character(2:10)) {
  plot.fcal <- rbind(
    plot.fcal,
    data.frame(
      time = calpro_time,
...
preds.crp <- predictY(models.crp[[10]], newdata = new.data, draws = TRUE)
plot.crp <- data.frame(
  time = crp_time,
...
```{R}
#| include: false
ids.shared <- intersect(models.fcal[[10]]$pprob$ids, models.crp[[10]]$pprob$ids)

fc.df <- subset(models.fcal[[10]]$pprob, ids %in% ids.shared)[,c(1, 2)]
colnames(fc.df) <- c("ids", "FCcluster")
crp.df <- subset(models.crp[[10]]$pprob, ids %in% ids.shared)[,c(1, 2)]
