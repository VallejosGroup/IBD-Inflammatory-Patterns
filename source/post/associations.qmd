...
wrap_elements(p_AT) + p_AT_mlr$plot_multi$AT + p_AT_mlr$plot_uni$AT
```
...
```{R}
myDF.fc_AT <- reshape2::melt(
  myDF.fc,
  id.vars = "ids",
  measure.vars = paste0("AT_line_", seq(1, 8)), variable.name = "Line") %>%
  drop_na(value)
myDF.fc_AT <- merge(myDF.fc_AT,
                    myDF.fc[, c("ids", "class", "diagnosis")],
                    by = "ids",
                    all.x = TRUE,
                    all.y = FALSE)
```
