...
myDF.fc$class <- factor(myDF.fc$class,
                        levels = 1:8,
...
myDF.fc_means$prob <- factor(myDF.fc_means$prob,
                          levels = c(paste0("prob", 1:8), "probmax"),
                          labels = c(paste0("FC", 1:8), "probmax"))
...
p_mlr <- myDF.fc %>%
  ...
  mlrPlot(var = c("diagnosis", "age", "sex", "baseline_fc"), class = "class")

aux <- myDF.fc %>%
  mutate(class = relevel(class, ref = "FC3")) %>%
  mlrPlot(var = c("followup_cut"), class = "class")
...
aux$plot_multi$`followup_cut(2,4]` +
  aux$plot_multi$`followup_cut(4,6]` +
  aux$plot_multi$`followup_cut(6,7]`

p_followup <- myDF.fc %>%
   subset(probmax > 0.5) %>%
  plotCat("followup_cut", class = "class") 
...
wrap_elements(p_diagnosis) +
  p_mlr$plot_multi$`diagnosisUlcerative Colitis` + p_mlr$plot_multi$`diagnosisIBDU` +
  patchwork::plot_annotation(tag_levels = "A", title = "Subjects prob > 0.5 - Multivariate CI") 
...
fc.uc.at <- reshape2::melt(dict.fc.uc,
                           ...
                           variable.name = "Line") %>%
  drop_na(value) %>%
  mutate(value = as.numeric(value/365.25))
