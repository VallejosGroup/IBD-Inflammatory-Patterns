...
dk <- read.csv(paste0(prefix,
                      "Denmark/2024-12-13/Fcal_association_tests_merge4_6.csv"),
               sep = ";")
dk <- dk %>% mutate(Var1 = plyr::mapvalues(Var1,
...
```{R}
myDF.fc <- myDF.fc %>%
  mutate(class_order_original = class_order,
         ...
         probmax_original = pmax(prob_order1,
                                 prob_order2,
                                 prob_order3,
                                 prob_order4,
                                 prob_order5,
  ...
  mutate(probmax = pmax(prob_order1,
                        prob_order2,
                        prob_order3,
                        prob_order4,
                        prob_order5,
...
p1 <- myDF.fc %>% 
  ...
  summarise(prop50 = 100 * mean(probmax_original > 0.5),
...
myDF.fc_means <- myDF.fc %>% 
  ...
  summarise(across(starts_with("prob_order_original"), function(x) mean(x, na.rm = TRUE)),
            .groups = "drop") %>%
  pivot_longer(cols = starts_with("prob_order_original"), names_to = "prob_order_original")
```
...
myDF.fc_means <- myDF.fc_means %>%
  mutate(prob_order_original = factor(prob_order_original,
                                      levels = paste0("prob_order_original_",
                                                      1:8),
...
p_mlr_all <- myDF.fc %>%
  mutate(class_order = relevel(class_order, ref = "FC1")) %>%
  mlrPlot(var = c("diagnosis", "age", "sex"),
          class = "class_order",
          extern = dk) 
```
...
p <- wrap_elements(p_diagnosis_all) +
  wrap_elements(p_mlr_all$plot_everything$`diagnosisUlcerative Colitis` + 
                  p_mlr_all$plot_everything$`diagnosisIBDU` +
...
myDF.fc %>%
  mutate(class_aux = ifelse(class_order %in% c("FC2"), class_order_original, "Other")) %>%
