...
(`r round(myDF_means$value[myDF_means$class == "Assigned to FC1" & myDF_means$followup_cut == "(0,2]" & myDF_means$prob == "FC1"], 3)`
versus
`r round(myDF_means$value[myDF_means$class == "Assigned to FC1" & myDF_means$followup_cut == "(0,2]" & myDF_means$prob == "FC6"], 3)`).
This is not unexpected, as FC1 and FC6 share similar patterns within the first
2 years.  

### Univariate associations with respect to cluster assignments
...
myDF <- myDF %>% 
  mutate(class_combined = ifelse(class %in% c(2, 3, 9), 
                                 "FC2-3-9", paste0("F", class)))
```

As a sensitivity analysis, we also consider restricting the analysis to only
consider individuals whose class assignment was less uncertain (with posterior
probability > 0.5).
...
```{R}
with(myDF, chisq.test(class_combined, sex)) %>%
  pander()

with(myDF[myDF$probmax > 0.5,], chisq.test(class_combined, sex)) %>%
  pander()
```
...
```{R}
aov(age ~ class_combined , data = myDF) %>%
  summary() %>%
  pander()

aov(age ~ class_combined , data = myDF[myDF$probmax > 0.5,]) %>%
  summary() %>%
  pander()
```
...
plotCat <- function(dat, var, labels, biomarker) {
  perc.table <- data.frame(
    location = character(),
    cluster = character(),
    perc = numeric()
  ...
  for (g in sort(unique(dat$class))) {
    temp.1 <- subset(dat, class ==  g)
    for (loc in labels) {
      perc <- nrow(subset(temp.1, eval(parse(text = var)) == loc)) / nrow(temp.1)
      perc.table <- rbind(
        perc.table,
        data.frame(
          eval(parse(text = var)) = loc,
          cluster = paste0(biomarker, g),
          perc = perc
        )
      )
    }
  ...
  for (i in 1:length(labels)){
    ...
    p <- P + perc.table %>%
      filter( eval(parse(text = var)) == labels[i]) %>%
      ggplot(aes(x = cluster, y = perc)) +
      geom_bar(stat = "identity", fill = "#C2F8CB", color = "#82C68F") +
  ...
  p <- p +
    patchwork::plot_layout(nrow = length(labels)) +
...
<!--- CAV: removed this as we have alternative plots that don't show individual
level data
