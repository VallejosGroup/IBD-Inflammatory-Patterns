...
```{R}
chisq.test(cd.pheno$class, cd.pheno$Location) %>%
  pander()
```
...
perc.table <- data.frame(location = character(),
                         cluster = character(),
                         perc = numeric())
for (g in 1:8) {
  temp.1 <- subset(cd.pheno, class == paste0("FC ", g))
...
```{R}
fisher.test(cd.pheno$class, cd.pheno$L4, workspace = 200000000) %>%
  pander()
```
...
perc.table <- data.frame(behaviour = character(),
                         cluster = character(),
                         perc = numeric())
for (g in 1:8) {  
  temp.1 <- subset(cd.pheno, class == paste0("FC ", g))
...
```{R}
fisher.test(cd.pheno$class, cd.pheno$Perianal, workspace = 20000000) %>%
  pander()
```
