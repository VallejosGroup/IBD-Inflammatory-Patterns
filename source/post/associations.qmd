...
#| fig-cap: "The relationship between posterior probability of cluster assignment and subject follow-up (defined as last FC observation within the study period)."
pprobs <- c()

temp <- model.fc$pprob
for (i in 1:nrow(temp)) {
  pprobs <- c(pprobs, temp[i, temp[i, "class"] + 2])
}

pprob.df <- data.frame(ids = temp$ids, pprobs = pprobs)
...
for (id in unique(fcal$ids)) {
  ...
  temp <- temp[order(temp$calpro_time, decreasing = TRUE), ]
  fup <- c(fup, temp[1, "calpro_time"])
  n.obs <- c(n.obs, nrow(temp))
}

df <- data.frame(ids = unique(fcal$ids), fup = fup, n.obs = n.obs)
p <- merge(df, pprob.df, by = "ids") %>%
  filter(n.obs <= 20) %>%
  ggplot(aes(x = fup, y = pprobs, color = n.obs)) +
  geom_point() +
  geom_smooth(method = "lm", formula = "y ~ x") +
  theme_minimal() +
  xlab("Follow-up time (years)") +
  ylab("Posterior probability") +
  ylim(0, NA) + viridis::scale_color_viridis(option = "viridis") 
...
#| fig-cap: "The relationship between posterior probability of cluster assignment and subject follow-up (defined as last FC observation within the study period) by cluster."
...
pprob.df <- data.frame(ids = temp$ids, pprobs = pprobs, clusters = temp$class)
...
myDF <- fcal %>%
  ...
  mutate(followup_cut = cut(followup, breaks = seq(0, 8, by = 2)),
...
myDF <- myDF %>% 
  mutate(probmax = pmax(prob1, prob2, prob3, prob4, 
                        prob5, prob6, prob7, prob7))

myDF_means <- myDF %>% 
  group_by(class, followup_cut) %>%
  summarise(across(starts_with("prob"), mean, na.rm = TRUE)) %>%
  pivot_longer(cols = starts_with("prob"), names_to = "prob")

myDF_means %>% subset(prob != "probmax") |>
...
myDF |>
  ...
  xlab("Cluster") +
  ylab("Cluster assignment probability") +
  facet_wrap(.~class, ncol = 4, nrow = 2) +
  theme_classic()
```
...
for (perc in percs) {
  included <- c(included,
                paste0(round(sum(pprob.df$pprobs > perc) / nrow(pprob.df), 4) * 100, "%"))
}
...
```{R}
pprob.df <- subset(pprob.df, pprobs > 0.5)
model.fc$pprob <- subset(model.fc$pprob, ids %in% pprob.df$ids)
```

We will use a threshold of greater than 0.5. 
...
#| warning: false
p1 <- dict.fc %>%
  mutate(class = factor(class, labels = paste("FC", seq(1, 8)))) %>%
