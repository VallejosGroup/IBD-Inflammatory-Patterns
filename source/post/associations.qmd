...
p2 <- myDF.fc %>%
  ...
  mutate(diagnosis = relevel(diagnosis, ref = "IBDU")) %>%
  filter(probmax_combined > 0.5) %>%
  mlrPlot(var = "diagnosis")

wrap_elements(p1) + p2$plot$`diagnosisUlcerative Colitis` + p2$plot$`diagnosisCrohn's Disease`

p3 <- myDF.fc %>%
  mutate(class_combined = relevel(class_combined, ref = "FC10 (n = 209)")) %>%
  mutate(diagnosis = relevel(diagnosis, ref = "IBDU")) %>%
  filter(probmax_combined > 0.5) %>%
  mlrPlot(var = c("diagnosis", "age", "sex", "baseline_fc"))

wrap_elements(p1) + p3$plot$`diagnosisUlcerative Colitis` + p3$plot$`diagnosisCrohn's Disease`
```

As seen above, the distribution of diagnosis types is not evenly distributed 
across clusters. In particular, we observe an enrichment of UC patients in 
FC5 and FC6, both of which are characterized by a slow decrease in FC 
(normalized after around 5 and 1 years, respectively). The same occurs for FC8
(Furthermore, CD patients
appear to be over-represented in the 


UC patients appear to be under-represented 
...
```{R}
with(myDF.fc, 
     chisq.test(class_combined, sex)) %>%
  pander()

with(myDF.fc[myDF.fc$probmax_combined > 0.5,], 
     chisq.test(class_combined, sex)) %>%
  pander()
```
