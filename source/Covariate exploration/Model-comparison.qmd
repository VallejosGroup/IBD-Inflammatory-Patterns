...
model.crp.original[[6]] <- readRDS(paste0(prefix, "cache/crp-ma/crp-6.RDS"))

model.crp.age.sex <- list()
model.crp.age.sex[[6]] <- readRDS(paste0(
  prefix,
  "/staging/sex+age/crp/",
  "age+sex/cache/crp-6.RDS"
))
model.crp.age <- list()
model.crp.age[[6]] <- readRDS(paste0(
  prefix,
  "/staging/sex+age/crp/",
  "age/cache/crp-6.RDS"
))
model.crp.sex <- list()
model.crp.sex[[6]] <- readRDS(paste0(
  prefix,
  "/staging/sex+age/crp/",
  "sex/cache/crp-6.RDS"
))
...
```{R}
forest.data <- summary(model.crp.original[[6]])
```
...
```{R}
forest.data <- summary(model.crp.sex[[6]])
```
...
```{R}
forest.data <- summary(model.crp.age[[6]])
```
...
```{R}
plot(model.crp.age[[6]])
```

### Age + sex model

Age is significant in 5/6 classes. Sex is significant in two classes. 

```{R}
forest.data <- summary(model.crp.age.sex[[6]])
```
...
perf <- data.frame(
  Model = "Original",
  AIC = model.crp.original[[6]]$AIC,
  BIC = model.crp.original[[6]]$BIC,
  loglik = model.crp.original[[6]]$loglik
)
...
perf <- rbind(
  ...
  data.frame(
    Model = "Age",
    AIC = model.crp.age[[6]]$AIC,
    BIC = model.crp.age[[6]]$BIC,
    loglik = model.crp.age[[6]]$loglik
  )
)
...
perf <- rbind(
  ...
  data.frame(
    Model = "Sex",
    AIC = model.crp.sex[[6]]$AIC,
    BIC = model.crp.sex[[6]]$BIC,
    loglik = model.crp.sex[[6]]$loglik
  )
)
