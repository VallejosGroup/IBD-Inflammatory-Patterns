...
```{R}
uc.pheno <- read_xlsx(paste0(prefix, "2024-10-23/UC-pheno.xlsx"),
                      range = cell_cols(1:4))
temp <- unique(subset(fcal, diagnosis == "Ulcerative Colitis")$ids)

uc.pheno <- subset(uc.pheno, ids %in% temp)
...
```{R}
dict <- dict %>%
  mutate(Extent = if_else(!is.na(Extent), paste0("E", Extent), NA))
```

```{R}
dict.fc.uc <- subset(dict,
                     diagnosis == "Ulcerative Colitis" & (ids %in% unique(fcal$ids)))


counts <- data.frame(table(dict.fc.uc$Extent, useNA = "ifany"))
...
counts <- data.frame(table(dict.fc.uc$Smoke, useNA = "ifany"))
colnames(counts) <- c("Previously smoked", "Frequency")
...
uc.bio.dates <- merge(uc.bio.dates,
                      ...
                      all.x = FALSE,
                      all.y = FALSE) %>%
  mutate(X1ST.LINE.START = X1ST.LINE.START - date.of.diag,
         X2ND.LINE.START = X2ND.LINE.START - date.of.diag,
         X3RD.LINE.START = X3RD.LINE.START - date.of.diag,
         X4TH.LINE.START = X4TH.LINE.START - date.of.diag,
         X5TH.LINE.START = X5TH.LINE.START - date.of.diag,
         X6th.line.start = X6th.line.start - date.of.diag,
         X7TH.LINE.START = X7TH.LINE.START - date.of.diag,
         X8TH.LINE.START = X8TH.LINE.START - date.of.diag)
...
```{R}
uc.bio <- uc.bio.all[, c(2, 20, 21, 23, 101)]

names(uc.bio) <- c("censor", "Treatment", "startDate", "stopDate", "ids")

uc.bio$Treatment <- toupper(uc.bio$Treatment)
uc.bio$Treatment <- factor(uc.bio$Treatment,
                           levels =  c("A",
                                       "F",
                                       "G",
                                       "I",
...
uc.bio$stopDate <- plyr::mapvalues(uc.bio$stopDate, "13/02/20223", "13/02/2022")

uc.bio <- fix_date_df(uc.bio, c("startDate", "stopDate"), excel = TRUE)
```
...
```{R}
#| label: fig-first-year-uc-fc
#| fig-cap: "First line therapy by year for ulcerative colitis cohort."
#| warning: false

dict.fc.uc <- subset(dict.fc, diagnosis == "Ulcerative Colitis")
dict.fc.uc <- merge(dict.fc.uc, uc.bio, all.x = TRUE, all.y = FALSE)
dict.fc.uc <- dict.fc.uc %>%
  mutate(advancedT = if_else(is.na(Treatment), 0, 1))
dict.fc.uc$firstY <- dict.fc.uc$advancedT

dict.fc.uc$timeToTherapy <- with(dict.fc.uc, startDate - date.of.diag) / 365.25

for (i in 1:nrow(dict.fc.uc)) {
  if (dict.fc.uc[i, "advancedT"] == 1) { # If been on AT
    # If been on AT later than one year of diag
    if (is.na(dict.fc.uc[i, "startDate"])) {
      dict.fc.uc[i, "firstY"] <- 0 
    } else if (dict.fc.uc[i, "startDate"] - dict.fc.uc[i, "date.of.diag"] > 365.25) {
      dict.fc.uc[i, "firstY"] <- 0 # Map to "No"
    }
  }
}
...
```{R}
temp <- dict.fc.uc %>%
  mutate(timeToTherapy = if_else(advancedT == 0, 7, as.numeric(timeToTherapy))) %>%
  mutate(advancedT = if_else(timeToTherapy > 7, 0, 1)) %>%
   mutate(timeToTherapy  = if_else(timeToTherapy  > 7, 7, timeToTherapy))
...
```{R}
dict.crp.uc <- subset(dict.crp, diagnosis == "Ulcerative Colitis")
dict.crp.uc <- merge(dict.crp.uc, uc.bio, all.x = TRUE, all.y = FALSE)
dict.crp.uc <- dict.crp.uc %>%
  mutate(advancedT = if_else(is.na(Treatment), 1, 0))
dict.crp.uc$firstY <- dict.crp.uc$advancedT

for (i in 1:nrow(dict.crp.uc)) {
  if (dict.crp.uc[i, "advancedT"] == 0) { # If been on AT
    # If been on AT later than one year of diag
    
    if (is.na(dict.crp.uc[i, "startDate"])) {
      dict.crp.uc[i, "firstY"] <- 1 
    } else if (dict.crp.uc[i, "startDate"] - dict.crp.uc[i, "date.of.diag"] > 365.25) {
      dict.crp.uc[i, "firstY"] <- 1 # Map to "No"
    }
  }
}

# Change to more interpretable value
dict.crp.uc$firstY <- as.logical(plyr::mapvalues(dict.crp.uc$firstY,
                                     from = c(0, 1),
                                     to = c(TRUE, FALSE)))

dict.crp.uc$timeToTherapy <- with(dict.crp.uc, startDate - date.of.diag) / 365.25
```
...
```{R}
temp <- dict.crp.uc %>%
  mutate(timeToTherapy = if_else(advancedT == 1,
                                 7,
                                 as.numeric(timeToTherapy))) %>%
  mutate(advancedT = if_else(advancedT == 1, 0, 1)) %>%
  mutate(advancedT = if_else(timeToTherapy > 7, 0, 1)) %>%
   mutate(timeToTherapy  = if_else(timeToTherapy  > 7, 7, timeToTherapy))
