...
```{R}
counts <- data.frame(table(uc.pheno$Extent, useNA = "ifany"))
...
```{R}
counts <- data.frame(table(uc.pheno$Smoke, useNA = "ifany"))
colnames(counts) <- c("Previously smoked", "Frequency")
...
```{R}
#| warning: false
uc.bio <- read.csv(paste0(prefix, "2024-10-17/UC-cleaned.csv"))

uc.bio <- uc.bio[, c(2, 20, 21, 23, 101)]
...
uc.bio <- uc.bio %>%
  mutate(startDate = if_else(startDate == "na", NA, startDate),
         stopDate = if_else(stopDate == "na", NA, stopDate))
