...
if (file.exists("/.dockerenv")) { # Check if running in Docker
  ...
  # mounted at /Volumes
  prefix <- "/Volumes/igmm/cvallejo-predicct/libdr/2024-10-03"
}

fcal <- read.csv(file.path(prefix, "fcal-cleaned.csv"))

# Extract from TRAK which also now introduces CRP.
labs <- read.csv(file.path(prefix, "markers-cleaned.csv"))
...
updated <- read.csv(file.path(prefix, "allPatientsNathanCleaned.csv"))
outcomes <- read.csv(file.path(prefix, "cd-cleaned.csv"))
```
...
There are a total of `r nrow(dict)` subjects in our dictionary before
filtering by inclusion/exclusion criteria. Amongst those, there are
...
are observed for diagnosis.

As can be seen in the below frequency table, two spellings of Crohn's
disease (with and without an apostrophe ("'")) and three names for IBDU
are used. There is also a subject reported to not have IBD.
...
<!--- CAV: we may remove this from the final website as it shows small counts ---> 
...
Firstly, we remove individuals that we classed as not IBD patients and those
with a missing date of diagnosis. 
...
labs.fcal <- labs.fcal[, c("ids", "COLLECTION_DATE", "TEST_DATA")] %>%
...
mapped to "1250". Any FC tests given an error code by the laboratory
have been removed.
...
## CAV: need to remove this table as shows <5 counts
table(fcal$calpro_result[which(is.na(as.numeric(fcal$calpro_result)))])
...
We consider measurements taken 30 days before diagnosis. As it can be seen in 
...
mydf <- fcal.sorted %>% 
  ...
  summarise(n.before.diag = sum(days.from.diag < -3*30),
            n.high.before.diag = sum(days.from.diag < -3*30 & calpro_result > 250)) %>%
...
aux <- mydf$ids[mydf$n.high.before.diag == 7]

fcal.sorted %>% subset(ids == 59302) 
# lots of high results 7 years prior diagnosis
# also: do we have duplicated measurements?
# individual is included, but we lost many measurementss
fcal.sorted %>% subset(ids %in% aux[1])
fcal.sorted %>% subset(ids %in% aux[2])
# high results about 1 year prior to diagnosis
# individual is excluded as there is no diagnostic time
fcal.sorted %>% subset(ids %in% aux[3]) 
# like individual 1
fcal.sorted %>% subset(ids %in% aux[4]) 
fcal.sorted %>% subset(ids %in% aux[5]) 
fcal.sorted %>% subset(ids %in% aux[6]) 
fcal.sorted %>% subset(ids %in% aux[7]) 


fcal.sorted %>% subset(ids %in% 9561) 
fcal.sorted %>% subset(ids %in% 18532) 
fcal.sorted %>% subset(ids %in% 35849) 

dict %>% subset(date.of.diag.outcomes - date.of.diag.updated > 365)

fcal.sorted %>% subset(ids %in% 778)  
fcal.sorted %>% subset(ids %in% 901)  
fcal.sorted %>% subset(ids %in% 1375)  
fcal.sorted %>% subset(ids %in% 1599)  

dict %>% subset(date.of.diag == as.Date("2000-01-01", "%Y-%m-%d"))



view(dict %>% subset(ids %in% aux))
```
...
We also reduce the dataset to only subjects with a diagnostic FC which
equates to subjects with a FCAL measurement within 3 months of diagnosis
$t \leq 0.25$
...
There are `r length(diagnostic)` subjects with a diagnostic FCAL
available.

### Minimum followup

We do not require a minimum follow-up for a subject as this may bias our
findings. For example, UC patients who undergo a proctocolectomy
...
We have also plotted number of FC measurements for each subject against
...
We require a subject has at least three FC measurements across the
...
There are `r length(diagnostic)` subjects with a diagnostic CRP
available.
