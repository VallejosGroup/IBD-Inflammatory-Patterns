...
fcal.pheno <- fix_date_df(fcal.pheno, "calpro_date")
...
fcal.pheno.pre <- subset(fcal.pheno, calpro_date < as.Date("2008-04-01"))
temp <- data.frame(COLLECTION_DATE = fcal.pheno.pre$calpro_date, 
                   RESULT_STATUS = NA,
                   TEST = NA,
                   TEST_CODE = NA,
                   TEST_DATA = fcal.pheno.pre$calpro_result,
                   TEST_SET = NA,
                   ids = fcal.pheno.pre$ids)

fcal <- rbind(fcal, temp)
rm(temp)
...
only consider subjects diagnosed after October 2004 and before 13 August 2019
