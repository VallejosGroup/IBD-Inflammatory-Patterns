...
library(ragg) # Better graphics device
...
# Create plot data first (safer approach)
fcal_diag_data <- fcal %>%
  group_by(ids) %>%
  filter(calpro_time == min(calpro_time)) %>%
  ungroup() %>%
  mutate(days_from_diag = calpro_time * 365.25)

# Try multiple rendering approaches to avoid segfault
plot_success <- FALSE

# Approach 1: Try with Cairo PNG device directly
if (!plot_success) {
  tryCatch({
    Cairo::CairoPNG("plots/fc-diagnostic-dist.png", 
                    width = 16 * 2 / 3 * 300, 
                    height = 6 * 300, 
                    dpi = 300)
    
    p <- ggplot(fcal_diag_data, aes(x = days_from_diag)) +
    ...
    dev.off()
    plot_success <- TRUE
    cat("Plot saved successfully with Cairo\n")
  }, error = function(e) {
    if (!is.null(dev.list())) dev.off()
    cat("Cairo approach failed:", e$message, "\n")
  })
}

# Approach 2: Try with basic png device if Cairo failed
if (!plot_success) {
  tryCatch({
    png("plots/fc-diagnostic-dist.png", 
        width = 16 * 2 / 3 * 300, 
        height = 6 * 300, 
        res = 300)
    ...
    dev.off()
    plot_success <- TRUE
    cat("Plot saved successfully with PNG\n")
  }, error = function(e) {
    if (!is.null(dev.list())) dev.off()
    cat("PNG approach failed:", e$message, "\n")
  })
}

# If both approaches failed, just display basic summary
if (!plot_success) {
  cat("Could not render plot due to graphics issues.\n")
  cat("Summary of diagnostic FC timing:\n")
  print(summary(fcal_diag_data$days_from_diag))
  # Create a simple plot for display only (no saving)
  p <- ggplot(fcal_diag_data, aes(x = days_from_diag)) +
    stat_density(geom = "line", color = "#20A39E", linewidth = 1) +
    theme_void() +
    labs(x = "Days from diagnosis", y = "")
}
```
...
crp_diag_data <- crp %>%
  ...
  ungroup() %>%
  mutate(days_from_diag = crp_time * 365.25)
...
if (!plot_success) {
  tryCatch({
    Cairo::CairoPNG("plots/crp-diagnostic-dist.png", 
                    width = 16 * 2 / 3 * 300, 
                    height = 6 * 300, 
                    dpi = 300)
    
    p <- ggplot(crp_diag_data, aes(x = days_from_diag)) +
    ...
    dev.off()
    plot_success <- TRUE
    cat("CRP plot saved successfully with Cairo\n")
  }, error = function(e) {
    if (!is.null(dev.list())) dev.off()
    cat("Cairo approach failed for CRP:", e$message, "\n")
  })
}
...
if (!plot_success) {
  tryCatch({
    png("plots/crp-diagnostic-dist.png", 
        width = 16 * 2 / 3 * 300, 
        height = 6 * 300, 
        res = 300)
    ...
    dev.off()
    plot_success <- TRUE
    cat("CRP plot saved successfully with PNG\n")
  }, error = function(e) {
    if (!is.null(dev.list())) dev.off()
    cat("PNG approach failed for CRP:", e$message, "\n")
  })
}

# If both approaches failed, just display basic summary
if (!plot_success) {
  cat("Could not render CRP plot due to graphics issues.\n")
  cat("Summary of diagnostic CRP timing:\n")
  print(summary(crp_diag_data$days_from_diag))
}
```
...
At this point, the data contains `r nrow(crp)` CRP measurements across all
`r length(unique(crp$ids))` individuals. 
...
# Use Cairo for PDF output
Cairo::CairoPDF("plots/crp-dist-ma.pdf", width = 8, height = 4.5)
print(p1 + p2)
dev.off()
```
...
# Use Cairo for PDF output
Cairo::CairoPDF("plots/fcal-dist.pdf", width = 8, height = 4.5)
print(p1 + p2)
dev.off()
```
...
# Use Cairo for PDF output
Cairo::CairoPDF("plots/crp-dist.pdf", width = 8, height = 4.5)
print(p3 + p4)
dev.off()
```
...
# Use Cairo for PDF output
Cairo::CairoPDF("plots/crp-processed-dist.pdf", width = 8, height = 4.5)
print(p5 + p6)
dev.off()
...
# Use Cairo for PDF output
Cairo::CairoPDF("plots/Sup-Figure-1.pdf", width = 8, height = 8)
print(p)
dev.off()
```
