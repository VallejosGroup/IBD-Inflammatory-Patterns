...
## Find optimal number of knots in a 6-cluster model:
Search from 1-4
Run the 2_fcal_m1.R script.
Run the 2_fcal_lcmm.R script 

Read in data:
Log transform and make sure time is in years
```{r}
...
knots.1 <- readRDS(paste0(path_project, "/GeneratedData/cache/fcal-1knot-6.RDS"))
knots.1.list <- list()
knots.1.list[[6]] <- knots.1

cairo_pdf(paste0(path_project, "/Illustrations/1-knot.pdf"), width = 9, height = 9)
spaghettiPlot(fcal, knots.1.list, G = 6, log = TRUE, tmax = 5, sizes = T, knots = TRUE, n.knots = 1, var.time = "t", knot.type = "equal")
invisible(dev.off())
...
knots.2 <- readRDS(paste0(path_project, "/GeneratedData/cache/fcal-2knots-6.RDS"))
knots.2.list <- list()
knots.2.list[[6]] <- knots.2

cairo_pdf(paste0(path_project, "/Illustrations/2-knots.pdf"), width = 9, height = 9)
spaghettiPlot(fcal, knots.2.list, G = 6, log = TRUE, tmax = 5, sizes = T, knots = TRUE, n.knots = 2, var.time = "t", knot.type = "equal")
invisible(dev.off())
...
knots.3 <- readRDS(paste0(path_project, "/GeneratedData/cache/fcal-3knots-6.RDS"))
knots.3.list <- list()
knots.3.list[[6]] <- knots.3

cairo_pdf(paste0(path_project, "/Illustrations/3-knots.pdf"), width = 9, height = 9)
spaghettiPlot(fcal, knots.3.list, G = 6, log = TRUE, tmax = 5, sizes = T, knots = TRUE, n.knots = 3, var.time = "t", knot.type = "equal")
invisible(dev.off())
...
knots.4 <- readRDS(paste0(path_project, "/GeneratedData/cache/fcal-4knots-6.RDS"))
knots.4.list <- list()
knots.4.list[[6]] <- knots.4

cairo_pdf(paste0(path_project, "/Illustrations/4-knots.pdf"), width = 9, height = 9)

spaghettiPlot(fcal, knots.4.list, G = 6, log = TRUE, tmax = 5, sizes = T, knots = TRUE, n.knots = 4, var.time = "t", knot.type = "equal")
invisible(dev.off())
...
knots.df <- tibble(Knots = c(1, 2, 3, 4),
                   loglik = c(knots.1$loglik,
                              knots.2$loglik,
                              knots.3$loglik,
                              knots.4$loglik),
                   AIC = c(knots.1$AIC,
...
## Based on 2 knots, validate the 8-cluster model
Run the 3_fcal_lcmm.R script, that is using the m1 file with two knots

## Load the models
```{r}
# set the number of groups
models.fcal <- list()
file.name <- paste0(path_project, "/GeneratedData/cache/fcal-2knots-8.RDS")
models.fcal[[8]] <- readRDS(file.name)
...
#| results: "asis"
  ...
  cat(paste0("##### G = ", 8, "\n"))
  output <- postprob(models.fcal[[8]])$result
...
#| results: "asis"
  cat(paste0("##### G = ", 8, "\n"))
  plot(models.fcal[[8]], shades = TRUE)
```
...
ggsave(paste0(path_project, "/Illustrations/Residual-plot-fcal-8ncs.pdf"),
       plot = p1 / p2,
       width = 8,
       height = 8,
       units = "in")
...
predClass <- models.fcal[[8]]$pprob[, c("lbnr", "class")]
temp <- merge(models.fcal[[8]]$pred, predClass, by = "lbnr")
...
ggsave(paste0(path_project,"/Illustrations/cluster-resids-8ncs.pdf"),
       ArrangedPlot,
       width = 8,
       height = 8,
       units = "in")
